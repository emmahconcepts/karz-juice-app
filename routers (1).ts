...
import { receiptsRouter }      from "./routers/receipts";      // clientReceipts
...
export const appRouter = router({
  ...
  clientReceipts: receiptsRouter,
  search:         searchRouter,
  orders:         ordersRouter,
  drawings:       drawingsRouter,

  // ── Dashboard (inline — simple enough to keep here) ───────────────────────
  dashboard: router({
    getDailyKPIs: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return {
          ...
          grossProfit:  parseFloat(dailySales) - expensesTotal,
        };
      }),
  ...
  functions: router({
    createFunction: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const contract = parseFloat(input.contractAmount);
        return db.createFunction({
          functionAccountId: 1,
          ...
          balanceRemaining: (contract - deposit).toString(),
          status:           deposit > 0 ? "partial" : "partial",
        });
      }),

    getAllFunctions: protectedProcedure.query(async () => db.getAllFunctions()),

    getFunctionById: protectedProcedure
      .input(z.object({ id: z.number() }))
      .query(async ({ input }) => db.getFunctionById(input.id)),
  ...
  expenses: router({
    recordExpense: protectedProcedure
      .input(z.object({
        expenseDate: z.date(),
        amount:      z.string(),
        category:    z.enum(["general","machine_hire","vehicle_hire","utilities","ingredients","packaging_materials","wages_salaries"]),
        accountId:   z.number(),
    ...
    getExpensesByDateRange: protectedProcedure
      .input(z.object({ startDate: z.date(), endDate: z.date() }))
      .query(async ({ input }) => db.getExpensesByDateRange(input.startDate, input.endDate)),
  ...
  machineHire: router({
    recordMachineHire: protectedProcedure
      .input(z.object({
        ...
        ownerName:       z.string(),
        costType:        z.enum(["daily","per_job"]),
        hirePeriodStart: z.date(),
        hirePeriodEnd:   z.date().optional(),
        costAmount:      z.string(),
      }))
      .mutation(async ({ input, ctx }) => {
        if (ctx.user.role !== "admin" && ctx.user.role !== "operations") throw new TRPCError({ code: "FORBIDDEN" });
    ...
    getMachineHiresByDateRange: protectedProcedure
      .input(z.object({ startDate: z.date(), endDate: z.date() }))
      .query(async ({ input }) => db.getMachineHiresByDateRange(input.startDate, input.endDate)),

    deleteMachineHire: protectedProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ ctx }) => {
        if (ctx.user.role !== "admin" && ctx.user.role !== "operations") throw new TRPCError({ code: "FORBIDDEN" });
        return { success: true };
  ...
  vehicleHire: router({
    recordVehicleHire: protectedProcedure
      .input(z.object({
        vehicleOwnerName: z.string(),
        purpose:          z.enum(["delivery","function","sourcing"]),
        hirePeriodStart:  z.date(),
        hirePeriodEnd:    z.date().optional(),
        cost:             z.string(),
    ...
    getVehicleHiresByDateRange: protectedProcedure
      .input(z.object({ startDate: z.date(), endDate: z.date() }))
      .query(async ({ input }) => db.getVehicleHiresByDateRange(input.startDate, input.endDate)),

    deleteVehicleHire: protectedProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ ctx }) => {
        if (ctx.user.role !== "admin" && ctx.user.role !== "operations") throw new TRPCError({ code: "FORBIDDEN" });
        return { success: true };
  ...
  // ── OCR Upload queue (legacy — kept for OCRModule.tsx compatibility) ───────
  receipts: router({
  ...
  products: router({
    ...
    createProduct: protectedProcedure
      .input(z.object({ name: z.string(), sku: z.string(), description: z.string().optional(), category: z.string().optional() }))
      .mutation(async ({ input, ctx }) => {
        if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
